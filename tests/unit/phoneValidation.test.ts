...
describe('phoneValidation - Utility Tests', () => {
  describe('validateGhanaPhoneNumber', () => {
    ...
    it('should validate Landline numbers correctly', () => {
      const result = validateGhanaPhoneNumber('0312345678');
      expect(result.isValid).toBe(true);
  ...
  describe('identifyOperator', () => {
    it('should identify MTN', () => {
      expect(identifyOperator('0241234567')).toBe('MTN');
    });

    it('should identify Vodafone', () => {
      expect(identifyOperator('0501234567')).toBe('VODAFONE');
    });

    it('should identify AirtelTigo', () => {
      expect(identifyOperator('0271234567')).toBe('AIRTELTIGO');
    });

    it('should return null for unrecognized prefix', () => {
      expect(identifyOperator('0991234567')).toBeNull();
    });
  });

  describe('convertToGhanaFormat', () => {
    it('should convert international to Ghana format', () => {
      expect(convertToGhanaFormat('+233241234567')).toBe('0241234567');
    });
  });

  describe('convertToInternationalFormat', () => {
    it('should convert Ghana to international format', () => {
      expect(convertToInternationalFormat('0241234567')).toBe('+233241234567');
    });
  });
});
