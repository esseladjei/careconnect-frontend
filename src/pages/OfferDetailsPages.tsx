...
const mockDoctor: DoctorDetails = {
  ...
  services: [
    ...
    'Rhythm Disorder Consultation',
  ],
  telemedicineOffer:
    'First 15-minute video consultation is discounted by 20% for new patients.',
};
...
const DoctorDetailPage: React.FC = () => {
  ...
  const selectedSlotFromSearch = (location.state as any)?.selectedSlot;

  const [selectedSlot, setSelectedSlot] = useState<string | null>(null);
  const [selectedDay, setSelectedDay] = useState(days[0]);
  const [isBookingConfirmed, setIsBookingConfirmed] = useState(false);

  // Pre-populate the selected slot if one was passed from search results
  useEffect(() => {
    if (selectedSlotFromSearch) {
      const timeString = new Date(
        selectedSlotFromSearch.start
      ).toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit',
      });
  ...
  const BookingWidget = () => (
    <div className="bg-white p-6 rounded-xl shadow-xl sticky top-4">
      ...
      {/* 2. Available Slots */}
      <h4 className="text-md font-semibold text-gray-700 mb-3 mt-4">
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-8">
        <div className="bg-white p-10 rounded-xl shadow-2xl text-center max-w-lg">
          ...
          <p className="text-xl text-gray-700 mb-6">
            Your appointment with **{mockDoctor.name}** is scheduled for **
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-6 rounded-xl shadow-lg">
              ...
              <div className="flex items-center space-x-6 mb-6 border-b pb-4">
                <div className="h-24 w-24 bg-blue-100 rounded-full flex items-center justify-center overflow-hidden shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xl text-blue-600 font-medium">
                    {mockDoctor.specialisation}
                  </p>
                  <div className="flex items-center text-yellow-500 mt-1">
                    <span className="mr-1">⭐</span>{' '}
                    {mockDoctor.rating.toFixed(1)} Rating
                  </div>
                </div>
              </div>
