...
const Navbar: React.FC = () => {
  ...
  // Example: get user ID from localStorage (replace with your actual auth logic)
        ...
        <div className="hidden md:flex items-center space-x-6">
          {navLinks.map((link) => (
            <div key={link.name} className="relative group">
              ...
              >
                {link.name}
                {link.submenu && <span className="ml-1">
        ...
        <div className="md:hidden mt-4 space-y-2 bg-blue-800 p-3 rounded-md">
          {navLinks.map((link) => (
            <div key={link.name}>
              ...
              >
                {link.name}
                {link.submenu && <span className={`transition-transform ${isAccountOpen ? 'rotate-180' : ''}`}>▼</span>}
              </button>
